...
export interface PanAndZoomHOCProps {
    ...
    onZoom?: (x: number | undefined, y: number | undefined, scale: number | undefined, event: Event) => void;
    onPanAndZoom?: (x: number, y: number, scale: number, event: Event) => void;
}

export default function panAndZoom<P>(WrappedComponent: React.SFC<P> | React.ComponentClass<P> | string | any): React.ComponentClass<Overwrite<P, PanAndZoomHOCProps>> {
    return class PanAndZoomHOC extends React.PureComponent<PanAndZoomHOCProps, never> {
        ...
        handleKeyDown = (event: KeyboardEvent) => {
          ...
          const y: number | undefined = this.props.y;
          const scale: number | undefined = this.props.scale;
          const speed: number | undefined = this.props.speed;
          const scaleFactor: number | undefined = this.props.scaleFactor;
          const minScale: number | undefined = this.props.minScale;
          const maxScale: number | undefined = this.props.maxScale;
          if (x !== undefined && y !== undefined && scale !== undefined && speed !== undefined && scaleFactor !== undefined && minScale !== undefined && maxScale !== undefined) {
              // const { deltaY } = event;
              let newScale
              if(event.keyCode === 187 || event.keyCode === 38){ // '+' or 'up' key
                newScale = scale + speed;
              }else if (event.keyCode === 189 || event.keyCode === 40){ // '-' or 'down' key
                newScale = scale - speed;
              } else {
                return
              }
              ...
              if (factor !== 1) {
                  ...
                  if (target !== null && 'getBoundingClientRect' in target) {
                      ...
                      if (onPanAndZoom) {
                          onPanAndZoom(x, y, scale + this.ds, event);
                      }
        ...
        handleMouseDown = (event: MouseEvent) => {
          console.log("handleMouseDown")
            ...
            if (event.button === 1) { //滚轮点击
                document.addEventListener('mousemove', this.handleScrollMouseMove);
                document.addEventListener('mouseup', this.handleScrollMouseUp);
            }
        };

        handleScrollMouseMove = (event: MouseEvent) => {
            console.log('scroll mousemove:', event)
            event.preventDefault();
        }

        handleScrollMouseUp = (event: MouseEvent) => {
            document.removeEventListener('mousemove', this.handleScrollMouseMove);
            document.removeEventListener('mouseup', this.handleScrollMouseMove);
            event.preventDefault();
        }
        ...
        normalizeTouchPosition(event: MouseEvent | TouchEvent, parent: Element | any) {
            const position = {
        ...
        render() {
            const { children, scaleFactor, x: tempX, y: tempY, scale: tempScale, minScale, maxScale, onPanStart, onPanMove, onPanEnd, onZoom, onPanAndZoom, renderOnChange, passOnProps, ignorePanOutside, wheelDisabled, ...other } = this.props;
            const x: number | undefined = this.props.x;
            const y: number | undefined = this.props.y;
            const scale: number | undefined = this.props.scale;
